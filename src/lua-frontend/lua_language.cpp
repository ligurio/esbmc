#include <lua-frontend/lua_language.h>
...
bool lua_languaget::parse(const std::string &path)
{
  (void)path;
  /* log_debug("lua", "Parsing: {}", path); */
...
bool lua_languaget::final(contextt &)
{
  return true;
}

bool lua_languaget::typecheck(contextt &context, const std::string &)
{
  // Load C models.
...
void lua_languaget::show_parse(std::ostream &out)
{
  out << "AST:\n";
  const std::string function = config.options.get_option("function");
  if (function.empty())
